...
class Api::V2::ElectorateController < Api::BaseController
  ...
  def index
    ...
    if @current_user.account.geography_electorates_id
      ...
      @federal_hor_leaders = LeaderProfile.where(geography_electorate_id: [@current_user.account.geography_electorates_id, nil], parliament: LeaderProfile.parliaments[:federal], type_of_leader: LeaderProfile.type_of_leaders[:house_of_representative]).as_json
    ...
    render json: { electorate: @electorate, census: @census_data, federalHorLeaders: @federal_hor_leaders, federalSenateLeaders: @federal_sen_leaders, stateLeaders: @state_leaders }, status: 200
  end
end
