...
class Api::V2::ElectorateController < Api::BaseController
  ...
  def index
    ...
    if @current_user.account.geography_electorates_id
      ...
      @federal_leaders = LeaderProfile.where(geography_electorate_id: [@current_user.account.geography_electorates_id, nil], parliament: LeaderProfile.parliaments[:federal]).as_json
